...
export const experience: {
...
}[] = [
  {
    ...
    bullets: [
      '박쥐 모양 로봇의 귀,코,발 구조에 힘줄 구동 방식(tendon-driven actuation) 시스템을 프로토타이핑 하여, 음향 연구 목적의 실감 있는 구조 변형 구현',
      'Teensy 마이크로컨트롤러용 임베디드 C++ 펌웨어 개발 — ADC, DAC, UART, SPI 직접 레지스터 조작으로 주변장치 응답 시간 2ms 이하 달성',
      'NVIDIA Jetson 보드와 마이크로컨트롤러 간 멀티 디바이스 시리얼 통신 프로토콜 설계 및 테스트 → 실험실 환경 기준 패킷 손실률 7% → 0% 감소',
      '트레이스 라우팅 최적화 적용 커스텀 PCB 레이아웃 설계 → 초기 설계 대비 액추에이터 신호 무결성 약 15% 개선',
    ],
    tags: [
      { label: 'Embedded', bullets: [0, 1] },
      { label: 'PCB 설계', bullets: [0] },
      { label: 'Actuator Control', bullets: [1] },
    ],
  },
]
